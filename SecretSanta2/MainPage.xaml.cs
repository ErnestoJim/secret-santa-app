...
namespace SecretSanta2
{
    public partial class MainPage : ContentPage
    {
        ...
        private readonly DrawStorageService _storage = new();
        ...
        {
            base.OnAppearing();
            await RefreshSavedDrawsAsync();
        }

        private async Task RefreshSavedDrawsAsync()
        {
            var names = await _storage.ListNamesAsync();
            SavedDrawsPicker.ItemsSource = names.ToList();
        }
        ...
        private async void OnSendEmailsClicked(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    var body = $"Hola {kv.Key.FullName},\n\nTe ha tocado regalar a: {kv.Value.FullName}.\nRecuerda: límite máximo 40€.\nLa fecha de entrega será el 24 de diciembre durante la cena Navideña.\n\nQue la magia, el cariño y la ilusión de la Navidad nos acompañen mientras preparamos este detallito especial.\n\n¡Felices fiestas!";
                    await _emailSender!.SendAsync(kv.Key.Email, "Tu amigo secreto familiar", body);
                }
                StatusLabel.TextColor = Colors.Green;
        ...
        private async void OnSaveDrawClicked(object sender, EventArgs e)
        {
            var name = DrawNameEntry.Text?.Trim();
            if (string.IsNullOrWhiteSpace(name))
            {
                StatusLabel.TextColor = Colors.Red;
                StatusLabel.Text = "Pon un nombre al sorteo.";
                return;
            }
            var draw = new SecretSantaDraw
            {
                Name = name,
                Participants = _participants.ToList(),
                Restrictions = _restrictions.ToList()
            };
            await _storage.SaveAsync(draw);
            await RefreshSavedDrawsAsync();
            StatusLabel.TextColor = Colors.Green;
            StatusLabel.Text = "Sorteo guardado.";
        }

        private async void OnLoadDrawClicked(object sender, EventArgs e)
        {
            if (SavedDrawsPicker.SelectedItem is not string name)
            {
                StatusLabel.TextColor = Colors.Red;
                StatusLabel.Text = "Selecciona un sorteo guardado.";
                return;
            }
            var draw = await _storage.LoadByNameAsync(name);
            if (draw == null)
            {
                StatusLabel.TextColor = Colors.Red;
                StatusLabel.Text = "No se encontró el sorteo.";
                return;
            }
            _participants.Clear();
            foreach (var p in draw.Participants) _participants.Add(p);
            _restrictions.Clear();
            foreach (var r in draw.Restrictions) _restrictions.Add(r);
            _assignments.Clear();
            DrawNameEntry.Text = draw.Name;
            RefreshPickers();
            StatusLabel.TextColor = Colors.Green;
            StatusLabel.Text = "Sorteo cargado.";
        }

        private async void OnDeleteDrawClicked(object sender, EventArgs e)
        {
            if (SavedDrawsPicker.SelectedItem is not string name)
            {
                StatusLabel.TextColor = Colors.Red;
                StatusLabel.Text = "Selecciona un sorteo a eliminar.";
                return;
            }
            await _storage.DeleteAsync(name);
            await RefreshSavedDrawsAsync();
            StatusLabel.TextColor = Colors.Green;
            StatusLabel.Text = "Sorteo eliminado.";
        }
